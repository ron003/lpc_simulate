...
typedef struct {							/*!< ETHERNET Structure */
	...
	__IO addr_t    DMA_REC_DES_ADDR;		/*!< Receive descriptor list address register */
	__IO addr_t    DMA_TRANS_DES_ADDR;		/*!< Transmit descriptor list address register */
 ...
 * functions.
 */
STATIC INLINE void Chip_ENET_RMIIEnable(LPC_ENET_T *pENET __attribute((__unused__)))
{
	LPC_CREG->CREG6 |= 0x4;
}
 ...
 * functions.
 */
STATIC INLINE void Chip_ENET_MIIEnable(LPC_ENET_T *pENET __attribute((__unused__)))
{
	LPC_CREG->CREG6 &= ~0x7;
}
											 ...
											 ENET_ENHTXDESC_T *pTXDescs, ENET_ENHRXDESC_T *pRXDescs)
{
	...
	pENET->DMA_TRANS_DES_ADDR = (addr_t) pTXDescs;
	pENET->DMA_REC_DES_ADDR = (addr_t) pRXDescs;
}
